...
const it=(d?.Items??[])[0];
console.log("item",it?.Id,it?.Nome);
for(const r of [`/api/Seguro/${it.Id}`,`/api/Seguro/${it.Id}?Guid=${guid}`]){
  const res=await chamarCompreFacil(r,{base});
  console.log(r,res.status,JSON.stringify(res.dados??"").slice(0,2500));
}
