...
function normalizeFlightBaggage(flight: any): any {
  ...
  const explicitPersonal = asBoolean(flight.personal_item ?? flight.personalItem);
  const explicitCarry = asBoolean(flight.carry_on ?? flight.carryOn ?? flight.hand_baggage);

  return {
    ...flight,
    // Regra comercial VIA AIR: todo aéreo do pacote inclui item pessoal e
    // bagagem de mão; a mala despachada diferencia LIGHT de STANDARD.
    personal_item: explicitPersonal ?? true,
    carry_on: explicitCarry ?? true,
    checked_bag: checked,
    fare_class:
