  ...
  .handler(async ({ data, context }): Promise<OrderDetail> => {
    ...
    return {
      order: {
        ...
        pixBaixaTipo: (order as { pix_baixa_tipo?: string | null }).pix_baixa_tipo ?? null,
        notes: order.notes,
  ...
  .handler(async ({ data, context }) => {
    ...
    if (normalized) {
      ...
      if (match?.id) {
        ...
        if (!match.passport_number && payload.passport_number) patch.passport_number = payload.passport_number;
        if (Object.keys(patch).length > 0) {
          await context.supabase.from("order_passengers").update(patch).eq("id", match.id);
        }
        return { id: match.id };
      }
    }
