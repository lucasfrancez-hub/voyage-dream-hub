...
export function buildHumanizacaoPrompt(nome?: string): string {
  ...
  return [
    ...
    `RECIPROCIDADE HUMANA: se a pessoa disser "tudo bem?", "tudo bom?", "como vc tá?" ou equivalente, responda explicitamente que vc está bem também ANTES de seguir o assunto (ex.: "Tô bem, obrigado! E vc?"). Nunca ignore a pergunta, nunca responda só "sim" e nunca pule direto pro roteiro. Vale para todos os agentes e também na apresentação inicial.`,
    `SAUDAÇÃO SECA ("oi", "bom dia", "boa noite" sem mais nada): cumprimente de volta JÁ PERGUNTANDO como a pessoa está — "Boa noite, <Nome>! Tudo bem?" — depois se apresente e pergunte como pode ajudar. Nunca cumprimente sem perguntar se está tudo bem.`,
