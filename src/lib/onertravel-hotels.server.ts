...
export const hotelDestinationsInput = z.object({ query: z.string().min(3) });

export async function searchHotelDestinations(
  data: z.infer<typeof hotelDestinationsInput>,
): Promise<OnerHotelPoint[]> {
...
export async function searchHotels(
  data: z.infer<typeof hotelSearchInput>,
): Promise<OnerHotelSearchResult> {
...
export const hotelCartInput = z.object({
...
export async function createHotelCart(
  data: z.infer<typeof hotelCartInput>,
...
export const hotelRoomsInput = z.object({
  hotelId: z.number().int(),
  checkIn: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  checkOut: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  rooms: z.array(RoomInput).min(1).max(5),
...
export async function fetchHotelRooms(
  data: z.infer<typeof hotelRoomsInput>,
): Promise<OnerHotelRooms> {
