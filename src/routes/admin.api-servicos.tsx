...
// Tela única e à prova de erro: cria o acesso de Serviços (Compre Fácil)
// sempre com nome/código fixos e apenas a permissão services:read.
const NOME_FIXO = "Sky Hub — Serviços Compre Fácil";
const CODIGO_FIXO = "SKY_HUB_SERVICOS";

export const Route = createFileRoute("/admin/api-servicos")({
  component: PainelTokenServicos,
  head: () => ({
    meta: [
      { title: "API — Serviços Compre Fácil | Admin Via Air" },
      { name: "robots", content: "noindex, nofollow" },
    ],
  }),
});

function PainelTokenServicos() {
  ...
  const tokensServicos = (clientes.data ?? []).filter(
    (c) => c.scopes.length === 1 && c.scopes[0] === "services:read",
  );
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-6 p-4 md:p-6">
      <header className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-semibold">API — Serviços Compre Fácil</h1>
          <p className="text-sm text-muted-foreground">
            Gera o token para o Sky Hub consultar serviços (transfer, passeio, ingresso, serviço e
            seguro). Só escolha o ambiente e clique em criar — as permissões já vêm corretas.
          </p>
        </div>
      </header>
      ...
      <section className="rounded-xl border bg-card p-4">
        <h2 className="text-sm font-semibold">Criar token de Serviços</h2>
        <p className="mt-1 text-xs text-muted-foreground">
          Nome: <strong>{NOME_FIXO}</strong> · Permissão: apenas consultar serviços
        </p>
      ...
      <section className="rounded-xl border bg-card">
        <h2 className="border-b px-4 py-3 text-sm font-semibold">Tokens de Serviços existentes</h2>
        {clientes.isLoading ? (
          <p className="p-4 text-sm text-muted-foreground">Carregando…</p>
        ) : !tokensServicos.length ? (
          <p className="p-4 text-sm text-muted-foreground">Nenhum token de serviços criado ainda.</p>
        ) : (
          <ul className="divide-y">
            {tokensServicos.map((c) => (
