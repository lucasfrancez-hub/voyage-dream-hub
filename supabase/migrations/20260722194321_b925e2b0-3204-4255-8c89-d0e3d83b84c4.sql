...
BEGIN
  ...
  room_type := COALESCE(snap->>'room_type', pkg.room_type);
  room_category := COALESCE(snap->>'room_category', pkg.room_category);
  bed_type := COALESCE(snap->>'bed_type', pkg.bed_type);
  ...
  IF hotel_name IS NOT NULL AND hotel_name <> '' THEN
    ...
    ELSE
      UPDATE public.order_items
       ...
       WHERE order_id = _order_id AND kind = 'hotel';
    END IF;
  END IF;
