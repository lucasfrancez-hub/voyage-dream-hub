...
BEGIN
  ...
  -- Só semeia pagamento quando o pedido vem de um pacote pronto (checkout).
...
-- Remove pagamentos "fantasma" que foram criados por engano em pedidos avulsos
DELETE FROM public.order_payments p
USING public.orders o
WHERE p.order_id = o.id
  AND o.package_id IS NULL
  AND p.description = 'Registrado no checkout'
  AND p.status = 'pending'
  AND p.amount = o.total_price
  AND NOT EXISTS (
    SELECT 1 FROM public.order_payments p2
    WHERE p2.order_id = o.id AND p2.id <> p.id
  );
