...
async function captureTab(tabId, windowId) {
  ...
  // captureVisibleTab fotografa a aba ativa da janela, não necessariamente a
  // aba que pediu a captura. Mantemos a reserva ativa durante o processo para
  // evitar capturar outra página ou receber um falso erro de permissão.
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg && msg.type === "viaair-capture") {
    ...
    captureQueue = captureQueue
      .then(() => captureTab(sender.tab.id, sender.tab.windowId))
