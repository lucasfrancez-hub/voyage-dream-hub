...
describe("ciclo 06h x 12h", () => {
  ...
  it("companhia", () =>
    expect(diffFare(base, { ...base, airline_iata: "G3" })).toContain("airline"));
  it("voo e tarifa", () => {
  ...
  it("dia BRT no formato ISO", () =>
    expect(curationDay(new Date("2026-08-13T02:00:00Z"))).toBe("2026-08-12"));
});
...
describe("checkpoint de datas reais", () => {
  it("preserva o total e retoma da próxima oportunidade pendente", () => {
    ...
    expect(
      datesProgress({ pendingLeads: pendingLeads as never, datesTotal: 55, datesDone: 38 }),
    ).toEqual({
      done: 38,
      total: 55,
    });
